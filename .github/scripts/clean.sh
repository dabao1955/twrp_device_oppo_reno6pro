...
if [[ ${retain_homebrew} != "true" ]]; then
  ...
  NONINTERACTIVE=1 ./uninstall-brew.sh -f -q 2
  sudo rm -rf -- ./uninstall-brew.sh 2
...
if [[ ${retain_docker_imgcache} != "true" ]]; then
  ...
  docker rmi -f $(docker images -q) 2
  echo "::endgroup::"
fi
if [[ ${retain_docker_buildkit} != "true" ]]; then
...
if [[ ${retain_vcs} != "true" ]]; then
  ...
  export AptPurgeList+=" gh vscode* node* npm* subversion mercurial"
...
if [[ ${retain_nodejs_npm} != "true" ]]; then
  ...
  sudo npm remove -g $(sed 's/^n$//g' <<<"$(jq -r ".node_modules[].name" /tmp/toolset.json)") &
...
_apt2purge_base() {
  ...
  sed -i.bak 's/'$'\u001b''//g;s/\[1;31m//g;s/\[0m//g' /tmp/apt2purge.{info,log} 2
}
# function apt2unset
_apt2unset() {
...
sudo apt autoremove -qy 2 || true
sudo rm -rf /var/cache/apt/archives/ 2 || true
echo "::endgroup::"

echo "::group:: {[-]}  Purging Unnecessary Files and Directories"
parallel 'echo -e "Purging {}" && sudo rm -rf -- {}' ::: ${DirPurgeList}
# Delete broken symlinks
sudo find /home/runner/.local/ /home/runner/ /usr/share/ /usr/bin/ /usr/local/bin/ /usr/local/share/ /usr/local/ /opt/ /snap/ -xtype l -delete 2
echo "::endgroup::"
